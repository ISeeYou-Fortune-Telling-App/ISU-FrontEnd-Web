...
export const RevenueChart: React.FC<{
  ...
  ({
    data,
    yAxisLabel = 'Giá trị',
    formatValue = (val: number) => val.toLocaleString('vi-VN'),
  }) => {
  ...
  (prevProps, nextProps) => {
    // Custom comparison: chỉ re-render khi data hoặc year thực sự thay đổi
    if (prevProps.year !== nextProps.year) return false;
    if (prevProps.data.length !== nextProps.data.length) return false;

    // So sánh deep data array
    ...
    return true; // Props giống nhau, không re-render
  },
);
...
export const TierPieChart: React.FC<{ data: Record<string, number>; title: string }> = React.memo(
  ({ data }) => {
