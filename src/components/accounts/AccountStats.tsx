...
import type { AccountStats as AccountStatsInterface } from '../../services/account/account.type';

interface StatCardAccountProps {
    value: number; // Nhận số nguyên
    label: string;
    colorClass: string;
}

const StatCardAccount: React.FC<StatCardAccountProps> = ({ value, label, colorClass }) => {
    // Logic định dạng số được giữ lại trong component này
        ...
        return (
            <>
                {Array(4).fill(0).map((_, index) => (
                    <div key={index} className="bg-gray-100 dark:bg-gray-700 h-24 rounded-xl animate-pulse"></div>
                ))}
            </>
        );
    }
